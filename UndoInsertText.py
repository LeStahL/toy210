...
class UndoInsertText(QUndoCommand):
    def __init__(self, parent, text_cursor, text):
        super(UndoInsertText, self).__init__()
        
        self.parent = parent
        self.cursor = text_cursor
        self.text = text.replace('\t', '    ')
        self.had_selection = self.cursor.hasSelection()
    ...
    def undo(self):
        ...
        self.cursor.movePosition(QTextCursor.Left, QTextCursor.KeepAnchor, self.text.size())
        self.cursor.removeSelectedText()
        self.cursor.clearSelection()
